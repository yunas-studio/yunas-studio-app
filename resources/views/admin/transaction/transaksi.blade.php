                                ...
                                <div class="search-box ms-2">
                                    <form action="{{ route('transaksi.index') }}" method="GET">
                                        <div class="position-relative">
                                            <input type="text" name="search" class="form-control rounded bg-light border-0" placeholder="Search..." value="{{ request('search') }}">
                        ...
                        <table class="table table-centered table-nowrap mb-0">
                            ...
                            <tbody>
                                @forelse($transactions as $transaksi)
                                    <tr>
                                        <td class="text-center"><a href="javascript: void(0);" class="text-body fw-bold">{{ $transaksi->receipt_code }}</a></td>
                                        <td class="text-center">{{ $transaksi->customer_name }}</td>
                                        <td class="text-center">{{ $transaksi->packet->name ?? 'N/A' }}</td>
                                            ...
                                            <form action="{{ route('transaksi.update-status', $transaksi->transaction_id) }}" method="POST">
                                                ...
                                                <select name="value" class="form-select form-select-sm" onchange="this.form.submit()">
                                                    @foreach (['belum dibayar', 'dp', 'sudah dibayar'] as $status)
                                                        <option value="{{ $status }}" {{ $transaksi->status == $status ? 'selected' : '' }}>{{ ucwords($status) }}</option>
                                                    @endforeach
                                                </select>
                                            </form>
                                        </td>
                                            ...
                                            <form action="{{ route('transaksi.update-status', $transaksi->transaction_id) }}" method="POST">
                                                ...
                                                <select name="value" class="form-select form-select-sm" onchange="this.form.submit()">
                                                     @foreach (['Siap Cetak', 'Proses Cetak', 'Selesai'] as $status)
                                                        <option value="{{ $status }}" {{ $transaksi->process_status == $status ? 'selected' : '' }}>{{ $status }}</option>
                                                    @endforeach
                                                </select>
                                            </form>
                                        </td>
                                                    ...
                                                    <div class="modal-content">
                                                        <div class="modal-body">
                                                            ...
                                                            <div class="p-4">
                                                                ...
                                                                <div class="py-2 mt-3 text-md-start"><h3 class="font-size-15 fw-bold">Order Summary</h3></div>
                                                                <div class="table-responsive">
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-3">
                                                {{-- I am assuming this button is for a future 'upload photo' feature --}}
                                                <a href="#" class="text-warning" data-bs-toggle="tooltip" title="Manage Photos">
                                                    <i class="uil uil-camera-change font-size-18"></i>
                                                </a>
                                                
                                                <a href="{{ route('transaksi.edit', $transaksi->transaction_id) }}" class="text-primary" data-bs-toggle="tooltip" title="Edit Transaction">
                                                    <i class="uil uil-pen font-size-18"></i>
                                                </a>
                                                
                                                <form action="{{ route('transaksi.destroy', $transaksi->transaction_id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this transaction?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-link text-danger p-0" data-bs-toggle="tooltip" title="Delete Transaction">
