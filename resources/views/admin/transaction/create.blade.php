        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-body">
                        {{-- ICON ADDED HERE --}}
                        <h5 class="font-size-20 mb-4"><i class="mdi mdi-file-document-edit-outline text-primary me-1"></i> Transaction Details</h5>
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                 <div class="mb-3">
                                    ...
                                    @error('packet_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="mb-3">
                        ...
                        <div id="additionals-container" class="mb-3">
                            <p class="text-muted">Select a packet to see default additionals.</p>
                        </div>
                        <div class="row mb-4">
                            <div class="col-md-8">
...
document.addEventListener('DOMContentLoaded', function() {
            ...
            <div class="row align-items-center mb-2 additional-row" id="additional-row-${id}">
                ...
                <div class="col-md-3">
                    <div class="input-group">
                        ...
                        <input type="text" name="additionals[${id}][price]" class="form-control price-input" value="${price}" readonly>
                    </div>
                </div>
                <div class="col-md-2">
                    <input type="number" name="additionals[${id}][quantity]" class="form-control quantity-input" value="${quantity}" min="1">
                </div>
                <div class="col-md-2">
                    <button type="button" class="btn btn-sm btn-danger remove-additional-btn w-100">X</button>
                </div>
            </div>`;

        if (additionalsContainer.querySelector('.text-muted')) {
    ...
    packetSelect.addEventListener('change', function() {
        ...
        additionalsContainer.innerHTML = '<p class="text-muted">Loading default additionals...</p>';
        if (!packetId) return;
        ...
        fetch(`/packets/${packetId}/default-additionals`)
            ...
            .then(data => {
                additionalsContainer.innerHTML = '';
                if (data.length === 0) {
                     additionalsContainer.innerHTML = '<p class="text-muted">No default additionals for this packet.</p>';
                } else {
                    data.forEach(item => addAdditionalRow(item.additional.id, item.additional.name, item.additional.price, item.quantity));
                }
                updateSummary();
            })
            .catch(() => additionalsContainer.innerHTML = '<p class="text-danger">Could not load additionals.</p>');
    });
    ...
    addBtn.addEventListener('click', function() {
        ...
        if (!id) return;
        addAdditionalRow(id, selected.dataset.name, selected.dataset.price, 1);
        addAdditionalSelect.value = '';
        updateSummary();
    });

    additionalsContainer.addEventListener('click', function(e) {
    ...
    additionalsContainer.addEventListener('input', function(e) {
